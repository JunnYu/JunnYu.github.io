...
(function(){
    ...
    var repoExp = new RegExp("^https://hub.fastgit.org/([^/]+)/([^/]+)(/(tree|blob)/([^/]+)(/(.*))?)?");
    var githubProvidedUrl = new RegExp("^https://api.github.com/.*");
    var githubDownloadUrl = new RegExp("^https://raw.fastgit.org/.*");
    var isBusy = false;
    ...
    var resolveUrl = function(repoUrl){
        ...
        if(matches && matches.length > 0){
            var root = (matches[5])? 
                "https://hub.fastgit.org/" + matches[1] + "/" + matches[2] + "/tree/" + matches[5] :
                repoUrl;
            return {
                author: matches[1],
                project: matches[2],
            ...
            if(matches && matches.length > 0){
                ...
                if(matches[4]){ // case: tree
                    ...
                    return new Promise(function(resolve, reject){
                        ...
                        .then(function(res){
                            var rootUrl = "https://hub.fastgit.org/" + results.author + "/" + results.project + "/tree/" + res.branch;
                            self.caches.push({
                                author: results.author,
                                project: results.project,
                                branch: res.branch,
                                pattern: "https://hub.fastgit.org/" + results.author + "/" + results.project + "/(tree|blob)/" + res.branch + "(/(.*))?",
                                rootUrl: rootUrl
                            });
                    ...
                    results.rootUrl = "https://hub.fastgit.org/" + results.author + "/" + results.project;
                    return Promise.resolve(results);
                }
            }
        ...
        if ( url ) {
            if ( githubDownloadUrl.test(url) ) return Promise.resolve(url);
                ...
                .then(function(resolved){
                    return "https://raw.fastgit.org/" + [resolved.author, resolved.project, resolved.branch, resolved.path].join("/");
                });
        } else return Promise.reject();
    };
    ...
    function createURL(pathToFolder, callbackScope){
        ...
        .then(function(resolved){
            if(!resolved.path){
                // root
                var durl = [
                    "https://hub.fastgit.org", resolved.author, resolved.project,
                ...
                if(resolved.type == "tree"){
                    ...
                    // for blob
                    downloadZip(
                        "https://raw.fastgit.org/" + [resolved.author, resolved.project, resolved.branch, resolved.path].join("/"),
                        callbackScope
                    );
                }
            }
