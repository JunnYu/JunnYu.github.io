...
downGitModule.factory('downGitService', [
    ...
    function ($http, $q) {
                ...
                for (var i = response.data.length - 1; i >= 0; i--) {
                    ...
                    if (response.data[i].type == "dir") {
                        ...
                        if (response.data[i].download_url) {
                            response.data[i].download_url = response.data[i].download_url.replace("raw.githubusercontent.com", "raw.fastgit.org")
        ...
        var getFile = function (path, url, files, requestedPromises, progress) {
            var promise = $http.request(url, { responseType: "arraybuffer", jsonp: "JSON_CALLBACK" }).then(function (file) {
        ...
        return {
            downloadZippedFiles: function (parameters, progress, toastr) {
                ...
                if (!repoInfo.resPath || repoInfo.resPath == "") {
                    ...
                    var downloadUrl = "https://hub.fastgit.org/" + repoInfo.author + "/" +
                    ...
                    $http.get(repoInfo.urlPrefix + repoInfo.resPath + repoInfo.urlPostfix).then(function (response) {
                        if (response.data instanceof Array) {
                            ...
                            target_url = response.data.download_url.replace("raw.githubusercontent.com", "raw.fastgit.org")
                            downloadFile(target_url, progress, toastr);
                        }

                    }, function (error) {
                        console.log("probable big file.");
                        downloadFile("https://raw.fastgit.org/" + repoInfo.author + "/" +
